...
//
#include "common/SystemSetup.hpp"
...
int filters()
{
	...
	GLuint programID = LoadShaders("StandardShading.vertexshader.GLSL", "StandardShading.fragmentshader.GLSL");
...
int normal_mapping() 
{
	...
	// Open a window and create its OpenGL context
	window = glfwCreateWindow(windowWidth, windowHeight, "Tutorial 13 - Normal Mapping", NULL, NULL);
	if (window == NULL) {
		fprintf(stderr, "Failed to open GLFW window. If you have an Intel GPU, they are not 3.3 compatible. Try the 2.1 version of the tutorials.\n");
		getchar();
		glfwTerminate();
	...
	// Create and compile our GLSL program from the shaders
	GLuint programID = LoadShaders("NormalMapping.vertexshader.GLSL", "NormalMapping.fragmentshader.GLSL");
...
{
	//Get config data
	SystemSetup mySysSetup;
	if (!mySysSetup.readfile())
	{
		printf("Error on config file\n");
		return -1;
	}
	ConfigProperties myConfData = mySysSetup.getall();
	if (myConfData.source == 1)
		normal_mapping();
	else if (myConfData.source == 2)
		to_texture();
	else if (myConfData.source == 3)
		alphachannel();
	else if (myConfData.source == 4)
		filters();
	return 0;
}